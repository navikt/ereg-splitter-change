...
open class AKafkaConsumer<K, V>(
            ...
            KafkaConsumer<K, V>(Properties().apply { config.forEach { set(it.key, it.value) } })
                .apply {
                    ...
                    tailrec fun loop(
                        ...
                        when {
                            PrestopHook.isActive() || !keepGoing -> (
                                if (PrestopHook.isActive()) {
                                    log.warn { "Kafka stopped consuming prematurely due to hook" }
                                    Unit
                                } else {
                                    Unit
                                }
                                ...
                                if (pollstate == Pollstate.RETRY) {
                                    ...
                                    log.info {
                                        "Kafka consumer - No records found on $topic, retry consumption in 60s. Retries left: $retries"
                                    }
    ...
    fun Pollstate.shouldContinue(): Boolean = this == Pollstate.RETRY || this == Pollstate.OK

    private fun <K, V> KafkaConsumer<K, V>.pollAndConsumption(
        pollDuration: Long,
        retryIfNoRecords: Boolean,
        doConsume: (ConsumerRecords<K, V>) -> KafkaConsumerStates,
    ): Pollstate =
        runCatching {
            ...
            log.error {
                "Failure during poll - ${it.message}, MsgHost: $currentConsumerMessageHost, Exception class name ${it.javaClass.name}\nMsgBoard: $kafkaConsumerOffsetRangeBoard \nGiven up on poll directly. Do not commit. Do not continue"
            }
            if (it is org.apache.kafka.common.errors.AuthorizationException) {
                // might be due to rotation of kafka certificates
                log.error { "Detected authorization exception (OZ). Should perform refresh" }
                ...
                // might be due to rotation of kafka certificates
                log.error { "Detected authentication exception (EC). Should perform refresh" }
            ...
            .let { cRecords ->
                ...
                val consumerState =
                    AKafkaConsumer.metrics.consumerLatency.startTimer().let { rt ->
                        runCatching { doConsume(cRecords) }
                            .onFailure {
                                log.error {
                                    "Failure during doConsume, MsgHost: $currentConsumerMessageHost - cause: ${it.cause}, message: ${it.message}. Stack: ${it.printStackTrace()}, Exception class name ${it.javaClass.name}\\"
                                }
                                if (it.message?.contains("failed to respond") == true ||
                                    it.message?.contains("terminated the handshake") == true
                                ) {
                            ...
                            }.getOrDefault(KafkaConsumerStates.HasIssues)
                            .also { rt.observeDuration() }
                    }
                when (consumerState) {
                    KafkaConsumerStates.IsOk -> {
                        try {
                            ...
                            if (hasConsumed) { // Only need to commit anything if records was fetched
                                commitSync()
                                if (!kafkaConsumerOffsetRangeBoard.containsKey("$currentConsumerMessageHost$POSTFIX_FIRST")) {
                                    kafkaConsumerOffsetRangeBoard["$currentConsumerMessageHost$POSTFIX_FIRST"] =
                                        Pair(cRecords.first().offset(), cRecords.last().offset())
                                }
                                kafkaConsumerOffsetRangeBoard["$currentConsumerMessageHost$POSTFIX_LATEST"] =
                                    Pair(cRecords.first().offset(), cRecords.last().offset())
                            }
                            Pollstate.OK
                        } catch (e: Exception) {
                            if (cRecords.count() > 0) {
                                kafkaConsumerOffsetRangeBoard["$currentConsumerMessageHost$POSTFIX_FAIL"] =
                                    Pair(cRecords.first().offset(), cRecords.last().offset())
                            }
                            log.error {
                                "Failure during commit, MsgHost: $currentConsumerMessageHost, leaving - ${e.message}" +
                                    ", Exception name: ${e.javaClass.name}\n" +
                                    "MsgBoard: $kafkaConsumerOffsetRangeBoard"
                            }
                            if (e.message?.contains("rebalanced") == true) {
                                // might be due to rotation of kafka certificates
                                log.error { "Detected rebalance/time between polls exception." }
                    ...
                    KafkaConsumerStates.HasIssues -> {
                        ...
                        log.error {
                            "Leaving consumer on HasIssues ErrorState: ${kErrorState.name} (specific error should be reported earlier), MsgHost: $currentConsumerMessageHost\nMsgBoard: $kafkaConsumerOffsetRangeBoard"
                        }
                        Pollstate.FAILURE
                    }
...
sealed class AllRecords<out K, out V> {
    ...
    data class Exist<K, V>(
        val records: List<Pair<Key<K>, Value<V>>>,
    ) : AllRecords<K, V>()
    ...
    fun getKeysValues(): List<KeyAndValue.Exist<out K, out V>> =
        if (this is Exist) {
            this.records
                .map {
    ...
    fun getKeysTombstones(): List<KeyAndTombstone.Exist<out K>> =
        if (this is Exist) {
            this.records
                .map {
    ...
    try {
        KafkaConsumer<K, V>(Properties().apply { config.forEach { set(it.key, it.value) } })
            .apply {
                ...
                c
                    .runCatching { seekToBeginning(emptyList()) }
                    .onFailure { log.error { "Failure during SeekToBeginning - ${it.message}" } }

                tailrec fun loop(
                    records: List<Pair<Key<K>, Value<V>>>,
                    retriesWhenEmpty: Int = 10,
                ): AllRecords<K, V> =
                    when {
                        PrestopHook.isActive() -> AllRecords.Interrupted
                            ...
                            val cr =
                                c
                                    .runCatching { Pair(true, poll(Duration.ofMillis(2_000)) as ConsumerRecords<K, V>) }
...
internal fun <K, V> getKafkaProducerByConfig(
    config: Map<String, Any>,
    doProduce: KafkaProducer<K, V>.() -> Unit,
): Boolean =
    try {
        KafkaProducer<K, V>(
...
fun <K, V> KafkaProducer<K, V>.sendNullKey(
    topic: String,
    value: V,
): Boolean =
    this
...
fun <K, V> KafkaProducer<K, V>.sendNullValue(
    topic: String,
    key: K,
): Boolean =
    this
