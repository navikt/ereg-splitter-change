...
class SalesforceClient(
    ...
    val sfPathSObjects = lazy { "/services/data/$SALESFORCE_VERSION/composite/sobjects" }
    ...
    private fun Response.parseAccessToken(): SFAccessToken =
        when (status) {
            Status.OK -> SFAccessToken.fromJson(bodyString())
                ...
                log.warn {
                    "Failed access token request at parseAccessToken- ${status.description} ($status) "
                }
    ...
    private fun getAccessTokenWithRetries(
        retry: Int = 1,
        maxRetries: Int = 4,
    ): SFAccessToken =
        httpClient.value.measure(accessTokenRequest, metrics.responseLatency).parseAccessToken().let {
            when (it) {
                is SFAccessToken.Missing -> {
                    if (retry > maxRetries) {
                        it.also { log.error { "Fail to fetch access token (including retries)" } }
                    } else {
    ...
    fun enablesObjectPost(doSomething: ((String) -> Response) -> Unit): Boolean {
        val doPostRequest: (String, SFAccessToken.Exists) -> Response = { b, at ->
            httpClient.value
                .measure(at.getPostRequest(sfPathSObjects.value).body(b), metrics.responseLatency)
                .also { metrics.postRequest.inc() }
        }

        fun doPost(
            id: String,
            at: SFAccessToken.Exists,
        ): Pair<Response, SFAccessToken> =
            doPostRequest(id, at).let { r ->
                log.debug {
                    "SF doPost initial response with http status - ${r.status} " // and body ${r.body} and headers ${r.headers}"
                }
                when (r.status) {
                    Status.UNAUTHORIZED -> {
                    ...
                    Status.CREATED -> {
                        ...
                        log.error {
                            "SF doPost issue with http status - ${r.status} " // and body ${r.body} and headers ${r.headers}"
                        }
